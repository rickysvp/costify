// 脚本：为所有消息添加 titleEn 字段
const fs = require('fs');
const path = require('path');

const filePath = path.join(__dirname, '../src/pages/Alerts.tsx');
let content = fs.readFileSync(filePath, 'utf8');

// 定义中文标题到英文标题的映射（按分类组织）
const titleMap = {
  // 预算与费用类 - Budget & Billing
...
Object.entries(titleMap).forEach(([cnTitle, enTitle]) => {
  ...
  // 查找 title: '中文标题' 后面没有 titleEn 的情况
  // 匹配模式：title: 'xxx',
  ...
  if (matches) {
    ...
    console.log(`✓ 已添加: ${cnTitle} -> ${enTitle} (${matches.length} 处)`);
  } else {
    // 检查是否已经有 titleEn
    const hasTitleEn = content.includes(`title: '${escapedTitle}',\n    titleEn:`);
    if (hasTitleEn) {
      skippedCount++;
...
console.log(`已存在跳过: ${skippedCount} 条`);
